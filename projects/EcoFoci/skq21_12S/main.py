...
def create_skq21_12S_sample_metadata():
    ...
    for faire_col, metadata_col in sample_mapper.mapping_dict[sample_mapper.related_mapping].items():
        ...
        if faire_col == 'samp_category' and metadata_col == sample_mapper.sample_metadata_sample_name_column:
            ...
            sample_metadata_results[faire_col] = sample_mapper.sample_metadata_df.apply(
                lambda row: sample_mapper.add_material_sample_id(metadata_row=row, cruise_code='SKQ21-15S'),
                axis=1
            )
            ...
            if faire_col == 'decimalLongitude':
                ...
                sample_mapper.sample_metadata_df['depth_from_pressure'] = max_depth
                ...
                final_max_depth = sample_mapper.sample_metadata_df.apply(
                    lambda row: sample_mapper.map_using_two_or_three_cols_if_one_is_na_use_other(metadata_row=row, 
                                                                                                desired_col_name='depth_from_pressure', 
    ...
    faire_sample_df_cruise_code_updated = sample_mapper.fix_cruise_code_in_samp_names(df=faire_sample_df_updated, 
                                                                                      cruise_code_to_replace='.SKQ2021',
                                                                                      replacement='.SKQ21-15S')
